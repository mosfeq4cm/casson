...
Pool = function(config) {
   ...
   */
  this.frameMesg = function (frame, callback) {

    var c = _one.getConnection();
  ...
  this.startUp = function() {
    ...
    _one.frameMesg(new Frame('Startup'), consoleErrorData);
  };
  ...
  /**
   * Set Pool.Connector and receive events
   * @method setConnector
  ...
  this.setConnector = function(spec) {
    ...
    c.on('end', function(uid) {
      console.log('on end:');
      delete connectors[uid] });
  };
   ...
   */
  this.sendForOptions = function() {

    _one.frameMesg(new Frame('Options'), consoleErrorData);
  };
   ...
   */
  this.setKeyspace = function(keyspace) {
    ...
    if (_one.keyspace) {
      _one.sendQuery(_one.keyspace.dbUse(), function(err, data) {
        if (!err) {
  ...
  this.sendCredentials = function() {
    ...
    _one.frameMesg(f, consoleErrorData);
  };
...
Pool.Connector = function(options) {
  ...
  this.conn.on('end', function() {
    ctor.emit('end', this.uuid);
    console.log(ctor.uuid + " disconnected");
  });
