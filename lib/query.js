...
/**
 * Truns a javascirpt object into Cassandra map
 * @class Map
 * @constructor
 * @param {Object} kv
 */
...
Batch = function(dml_statements, type) {
  ...
  if (type.toLowerCase === 'counter')
    this.bstr += 'COUNTER\n';
  else if (type.toLowerCase === 'unlogged')
    this.bstr += 'UNLOGGED\n';
  else
    this.bstr += 'BATCH\n';

  for (var dml in this.dmls) {
    this.bstr  += '\t' + dml + '\n';
  }
  this.bstr += 'apply batch';
